...
export async function GET(req: Request) {
  try {
    ...
    if (!month || !year) {
      return new NextResponse("Month and year are required query parameters", { status: 400 });
    }
    ...
    const articlesWithLogos = await Promise.all(articles.map(async (article) => {
      if (article.company_info && article.company_info.logo_id) {
        const logoBase64 = await getLogoAsBase64(article.company_info.logo_id, bucket);
        return {
          ...article,
          company_info: {
...
async function getLogoAsBase64(logoId: ObjectId, bucket: GridFSBucket): Promise<string | null> {
    try {
