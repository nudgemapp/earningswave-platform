...
const TickerSearch: React.FC<TickerSearchProps> = ({ handleFutureEarningsClick }) => {
  ...
  const handleSelect = async (ticker: Ticker) => {
    const matchingReport = data?.reports.find((report: ProcessedReport) =>
      report.symbol === ticker.symbol
    );
              ...
              <div className="space-y-1">
                {tickers.map((ticker) => (
                  ...
                  >
                    ...
                    <span className="text-gray-500 text-sm truncate">{ticker.name}</span>
                  </div>
                ))}
              </div>
            )}
