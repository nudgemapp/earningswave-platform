...
export async function GET(request: Request) {
  try {
    ...
    //   `https://finnhub.io/api/v1/calendar/earnings?from=${from}&to=${to}&token=${process.env.NEXT_PUBLIC_FINNHUB_API_KEY}`
    // );
    ...
    const transcriptListResponse = await fetch(
      `https://finnhub.io/api/v1/stock/transcripts/list?symbol=${ticker}&token=${process.env.NEXT_PUBLIC_FINNHUB_API_KEY}`
    );
    ...
    const transcriptId = "NVDA_3393990";
